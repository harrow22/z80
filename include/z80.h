 ...
 * \tparam Hardware To use the CPU it must be provided an object w/ the following 6 member functions.
 * The rest of what it contains is up to the developer.
 *  - u8 read8(u16 addr)
...
public:
    ...
    /**
     * \brief Constructor
     * \param hw a reference to an instance of type Hardware
     */
    explicit z80(Hardware& hw) : hardware {hw} {};
     ...
     * \return a negative number representing the the amount of cycles the cpu exceeded while running
     */
    int run(int cycles);

    /**
     * \brief Step the cpu forward a single instruction.
     */
    void step();
    ...
    /**
     * \brief Flags are stored separately, to get them as a single status byte they must be or'd together.
     ...
     */
    static void setf(Flags& f, u8 val);
...
private:
    ...
    {
        switch (mode) {
            case AddressMode::Extended: hardware.write8(fetch16(), val); return;
            case AddressMode::IndexedIX: {
                wz = static_cast<u16>(static_cast<s8>(fetch8()) + ix);
                hardware.write8(wz, val);
                return;
            ...
            case AddressMode::RegisterIndirectBC: hardware.write8(regs.b << 8 | regs.c, val); return;
            case AddressMode::RegisterIndirectDE: hardware.write8(regs.d << 8 | regs.e, val); return;
            case AddressMode::RegisterIndirectHL: hardware.write8(regs.h << 8 | regs.l, val); return;
            case AddressMode::RegisterIndirectSP: hardware.write8(sp, val); return;
            default: return; // quiets warnings, will never get here
        }
    }
    ...
    u16 getOperand()
    {
        switch (mode) {
            ...
            case AddressMode::Extended: return hardware.read8(fetch16());
            case AddressMode::IndexedIX: {
                wz = static_cast<u16>(static_cast<s8>(fetch8()) + ix);
                return hardware.read8(wz);
            } case AddressMode::IndexedIY: {
                wz = static_cast<u16>(static_cast<s8>(fetch8()) + iy);
                return hardware.read8(wz);
            ...
            case AddressMode::RegisterIndirectBC: return hardware.read8(pair(regs.b, regs.c));
            case AddressMode::RegisterIndirectDE: return hardware.read8(pair(regs.d, regs.e));
            case AddressMode::RegisterIndirectHL: return hardware.read8(pair(regs.h, regs.l));
            case AddressMode::RegisterIndirectSP: return hardware.read8(sp);
            case AddressMode::Null: return 0;
            default: return 0; // quiets warnings, will never get here
        }
    }
    ...
    bool getCondition()
    {
        switch (cond) {
            ...
            default: return false; // quiets warnings, will never get here
        }
    }
    ...
    void ldWZ()
    {
        if (src == AddressMode::Accumulator and dst == AddressMode::Extended)
            wz = hardware.read16(pc) + 1; // MEMPTR = addr + 1
        else if (src == AddressMode::Extended and dst == AddressMode::Accumulator)
            wz = pair(a, (hardware.read16(pc) + 1) & 0xFF); // MEMPTR_low = (addr + 1) & #FF,  MEMPTR_hi = A
            ...
            wz = static_cast<s8>(hardware.read8(pc)) + ix;
        else if (src == AddressMode::IndexedIY or dst == AddressMode::IndexedIY)
            wz = static_cast<s8>(hardware.read8(pc)) + iy;
    }
    ...
    [[nodiscard]] u8 fetch8() { return hardware.read8(pc++); }
    [[nodiscard]] u16 fetch16() { pc += 2; return hardware.read16(pc - 2); };
    ...
    u16 top()
    {
        const u16 top {hardware.read16(sp)};
        sp += 2;
        return top;
    }
    ...
    void pushpc()
    {
        hardware.write8(--sp, pc >> 8U);
        hardware.write8(--sp, pc & 0xFFU);
    }
    ...
    void storeBitResult(const u8 val)
    {
        ...
        if (mode1 == AddressMode::IndexedIX) {
            hardware.write8(static_cast<s8>(hardware.read8(--pc)) + ix, val);
            pc += 2;
        } else if (mode1 == AddressMode::IndexedIY) {
            hardware.write8(static_cast<s8>(hardware.read8(--pc)) + iy, val);
            pc += 2;
        }
    ...
    {
        const u8 tmp {hardware.read8(pair(regs.h, regs.l))};
        hardware.write8(pair(regs.d, regs.e), tmp);
    ...
    {
        const u8 subtrahend {hardware.read8(pair(regs.h, regs.l))};
    ...
    {
        const u8 tmp {hardware.input(this, regs.c)};
        hardware.write8(pair(regs.h, regs.l), tmp);
    ...
    {
        const u8 tmp {hardware.read8(pair(regs.h, regs.l))};
        hardware.output(this, regs.c, tmp);
    ...
    void ldAddrRp()
    {
        ...
        hardware.write16(addr, getOperand<src>());
        wz = addr + 1;
        flagsWereModified = false;
    }
    ...
    {
        const u16 addr {fetch16()};
        setOperand<dst>(hardware.read16(addr));
        wz = addr + 1;
        flagsWereModified = false;
    }
    ...
    void push()
    {
        ...
        hardware.write8(--sp, operand >> 8U);
        hardware.write8(--sp, operand & 0xFFU);
        flagsWereModified = false;
    }
    ...
    void ex2()
    {
        ...
        setOperand<mode>(pair(hardware.read8(sp + 1), hardware.read8(sp)));
        hardware.write16(sp, tmp);
        wz = getOperand<mode>();
        flagsWereModified = false;
    }
    ...
    void inc8()
    {
        ...
        if (mode == AddressMode::IndexedIX)
            hardware.write8(static_cast<s8>(hardware.read8(pc - 1)) + ix, res);
        else if (mode == AddressMode::IndexedIY)
            hardware.write8(static_cast<s8>(hardware.read8(pc - 1)) + iy, res);
        else
            setOperand<mode>(res);
        flagsWereModified = true;
    }
    ...
    void rld()
    {
        const u8 byte {hardware.read8(pair(regs.h, regs.l))};
        hardware.write8(pair(regs.h, regs.l), (byte << 4U) | (a & 0x0FU));
        a = (a & 0xF0U) | (byte >> 4U);
        rotateFlags(a);
    ...
    {
        const u8 byte {hardware.read8(pair(regs.h, regs.l))};
        hardware.write8(pair(regs.h, regs.l), (a << 4U) | (byte >> 4U));
    ...
    { // bit n, (hl) sets xf and yf based on value in the z80's internal wz (or memptr) register
        ...
        const u8 operand {hardware.read8(hl)};
        const u8 res {static_cast<u8>(operand & mask)};
        flags.hf = hbit;
        flags.nf = 0;
        flags.sf = res & sbit;
    ...
    void in()
    {
        const u8 c {hardware.input(this, regs.c)};
    ...
    void out()
    {
        hardware.output(this, regs.c, getOperand<mode>());
    ...
    void preIndexBit()
    {
        ...
        const u8 opcode {hardware.read8(pc--)}; // subtract one so next instruction will get the displacement byte
...
int z80<Memory>::run(const int rq)
{
    requested = rq;
...
void z80<Memory>::step()
{
    ...
    if (intRequested & iff1) {
        ...
        if (imode == 0) {
            (this->*instruction[ivector])();
            requested -= cycles[ivector] - 2;
        } else if (imode == 1) {
            rst<0x38>();
            requested -= 13;
        } else if (imode == 2) {
            pushpc();
            pc = wz = hardware.read16((i << 8U) | ivector);
            flagsWereModified = false;
            requested -= 19;
        }
        return;
...
std::string z80<Memory>::toString(const unsigned long long cycle) const
{
    return std::format(
            ...
            sp, ix, iy, i, r,
            hardware.read8(pc),
            hardware.read8(pc + 1U),
            hardware.read8(pc + 2U),
            hardware.read8(pc + 3U),
            cycle);
}
